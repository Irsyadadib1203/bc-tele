...
export async function POST(request: Request) {
  ...
  if (
    ...
    typeof settings.targetChatId !== "string" ||
    !settings.botToken ||
    !settings.targetChatId
  ) {
    return NextResponse.json(
      { error: "Bot Token dan Target Chat ID harus dikonfigurasi." },
      { status: 400 },
    );
  }
  ...
  if (!products.length)
    return NextResponse.json(
      { error: "Tidak ada produk setelah filter prefix diterapkan." },
      { status: 400 },
    );
  ...
  try {
    if (settings.broadcastFormat === "text") {
      ...
      await db.activityLog.create({
        data: {
          type: "BROADCAST",
          message: `Broadcast teks ${title} berhasil dikirim (${products.length} produk)`,
          meta: { categoryId: body.categoryId },
        },
      });
      return NextResponse.json({
        message: `Daftar harga ${title} berhasil dikirim dalam ${messages.length} pesan`,
      });
    }
    ...
    await telegramRequest(`${telegramUrl}/sendPhoto`, {
      method: "POST",
      body: formData,
    });
    ...
    await db.activityLog.create({
      data: {
        type: "BROADCAST",
        message: `Broadcast gambar ${title} berhasil dikirim (${products.length} produk)`,
        meta: { categoryId: body.categoryId },
      },
    });
    return NextResponse.json({
      message: `Broadcast ${title} berhasil dikirim`,
    });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Gagal mengirim Telegram";
    await db.activityLog.create({
      data: { type: "ERROR", message: `Broadcast ${title} gagal: ${message}` },
    });
