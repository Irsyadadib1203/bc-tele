...
import type { CustomBroadcast } from "@/components/custom-broadcast-manager";
...
export default async function SchedulesPage() {
  ...
  const [schedules, customBroadcasts] = level ? await Promise.all([db.broadcastSchedule.findMany({ where: { levelId: level.id } }), db.customBroadcast.findMany({ where: { levelId: level.id } })]) : [[], [] as CustomBroadcast[]];
