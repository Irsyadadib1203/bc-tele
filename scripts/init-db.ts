...
async function main() {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS \`PriceLevel\` (id VARCHAR(191) PRIMARY KEY, name VARCHAR(100) NOT NULL, isActive BOOLEAN DEFAULT FALSE, apiKey TEXT, targetChatId TEXT, targetMainChatId TEXT, targetPersonalChatId TEXT, priceChangeTargetGroup VARCHAR(10) DEFAULT 'main', priceTextTargetGroup VARCHAR(10) DEFAULT 'main', priceImageTargetGroup VARCHAR(10) DEFAULT 'main', scheduleEnabled BOOLEAN DEFAULT TRUE, feeEnabled BOOLEAN DEFAULT FALSE, feeSmall INT DEFAULT 5, feeMedium INT DEFAULT 10, feeLarge INT DEFAULT 25, feeOverrides TEXT, caption TEXT, imageCaption TEXT, headerTitle VARCHAR(255) DEFAULT 'PRICE UPDATE', primaryColor VARCHAR(20) DEFAULT '#5B5BD6', accentColor VARCHAR(20) DEFAULT '#A78BFA', createdAt DATETIME DEFAULT CURRENT_TIMESTAMP)`);
  await pool.query(`CREATE TABLE IF NOT EXISTS \`ProductCategory\` (id VARCHAR(191) PRIMARY KEY, levelId VARCHAR(191) NOT NULL, title VARCHAR(255) NOT NULL, type VARCHAR(100), selected BOOLEAN DEFAULT FALSE, priceChangeBroadcastEnabled BOOLEAN DEFAULT FALSE, prefixFilterEnabled BOOLEAN DEFAULT FALSE, excludedPrefixes TEXT, productCount INT DEFAULT 0, products JSON NOT NULL, syncedAt DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY level_title(levelId,title))`);
  await pool.query(`CREATE TABLE IF NOT EXISTS \`CustomBroadcast\` (id VARCHAR(191) PRIMARY KEY, levelId VARCHAR(191), name VARCHAR(100) NOT NULL, content TEXT NOT NULL, targetGroup VARCHAR(10) DEFAULT 'main', createdAt DATETIME DEFAULT CURRENT_TIMESTAMP)`);
  ...
  await addColumnIfMissing("PriceLevel", "targetChatId", "TEXT");
  await addColumnIfMissing("PriceLevel", "targetMainChatId", "TEXT");
  await addColumnIfMissing("PriceLevel", "targetPersonalChatId", "TEXT");
  await addColumnIfMissing("PriceLevel", "priceChangeTargetGroup", "VARCHAR(10) DEFAULT 'main'");
  await addColumnIfMissing("PriceLevel", "priceTextTargetGroup", "VARCHAR(10) DEFAULT 'main'");
  await addColumnIfMissing("PriceLevel", "priceImageTargetGroup", "VARCHAR(10) DEFAULT 'main'");
  await addColumnIfMissing("PriceLevel", "scheduleEnabled", "BOOLEAN DEFAULT TRUE");
  ...
  await addColumnIfMissing("CustomBroadcast", "levelId", "VARCHAR(191)");
  await addColumnIfMissing("CustomBroadcast", "targetGroup", "VARCHAR(10) DEFAULT 'main'");
  // Disabled by default so automatic price-change notices are only sent for
  // categories that the user explicitly enables in the Products page.
  await addColumnIfMissing("ProductCategory", "priceChangeBroadcastEnabled", "BOOLEAN DEFAULT FALSE");
  ...
  await pool.query("UPDATE `PriceLevel` AS levelItem JOIN `Settings` AS settingsItem ON settingsItem.id = 1 SET levelItem.targetChatId = COALESCE(levelItem.targetChatId, settingsItem.targetChatId) WHERE levelItem.targetChatId IS NULL");
  // Preserve existing delivery behaviour during migration: all former targets
  // become Main targets and all routes default to Main. The user can then move
  // individual IDs to Personal targets for each independent level.
  await pool.query("UPDATE `PriceLevel` SET targetMainChatId = targetChatId WHERE targetMainChatId IS NULL AND targetChatId IS NOT NULL");
  ...
  const selectedLevelId = (await db.settings.findUnique())?.selectedLevelId;
  if (typeof selectedLevelId === "string") {
    // Legacy schedules and custom broadcasts were global. Assign them once to
    // the level that was active before this migration so they can be edited
    // and then remain independent from other levels.
    await pool.execute("UPDATE `BroadcastSchedule` SET levelId = ? WHERE levelId IS NULL", [selectedLevelId]);
    await pool.execute("UPDATE `CustomBroadcast` SET levelId = ? WHERE levelId IS NULL", [selectedLevelId]);
    if ((await db.customBroadcast.count({ where: { levelId: selectedLevelId } })) === 0) {
      await db.customBroadcast.create({ data: { levelId: selectedLevelId, name: "BC Custom 1", content: "" } });
    }
  }
}
