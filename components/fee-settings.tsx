...
export function FeeSettings({ level }: FeeSettingsProps) {
  ...
  function addOverride() {
    ...
    if (!/^\d+(?:\.\d+)?$/.test(value) || !Number.isInteger(fee) || fee < 0) { setFailed(true); setNote("Denom harus berupa angka dan fee harus bilangan bulat ≥ 0"); return; }
    setPending({ title: "Konfirmasi override fee", message: `Tambahkan override denom ${value} dengan fee Rp${fee}?`, run: () => { setOverrides((current) => [...current.filter((item) => item.denom !== value), { denom: value, fee }]); setDenom(""); setOverrideFee(""); setFailed(false); setNote("Override fee ditambahkan. Simpan pengaturan untuk menerapkan perubahan."); } });
  }
