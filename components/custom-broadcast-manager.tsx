...
export type CustomBroadcast = { id: string; levelId: string; name: string; content: string; targetGroup: "main" | "personal" | "both" };
const targetOptions = [{ value: "main", label: "Chat utama" }, { value: "personal", label: "Chat pribadi" }, { value: "both", label: "Chat utama dan pribadi" }] as const;
...
export function CustomBroadcastManager({ initial, levelId, levelName }: { initial: CustomBroadcast[]; levelId: string | null; levelName: string }) {
  ...
  return <>
    ...
    {adding && <AddCustomBroadcast onClose={() => setAdding(false)} onCreate={async (name, content, targetGroup) => { try { const data = await call({ action: "create", levelId, name, content, targetGroup }); setItems((current) => [...current, data.item]); setAdding(false); setFailed(false); setNote(data.message); } catch (error) { setFailed(true); setNote(error instanceof Error ? error.message : "Gagal menambahkan BC custom"); } }} />}
  </>;
}
...
function CustomBroadcastCard({ item, canDelete, onChange, onSave, onSend, onDelete }: { item: CustomBroadcast; canDelete: boolean; onChange: (item: CustomBroadcast) => void; onSave: (item: CustomBroadcast) => void; onSend: (item: CustomBroadcast) => void; onDelete: () => void }) {
  return <article className="card custom-broadcast-card">
    ...
    <label className="field">Kirim ke<select value={item.targetGroup} onChange={(event) => onChange({ ...item, targetGroup: event.target.value as CustomBroadcast["targetGroup"] })}>{targetOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label>
...
function AddCustomBroadcast({ onClose, onCreate }: { onClose: () => void; onCreate: (name: string, content: string, targetGroup: CustomBroadcast["targetGroup"]) => void }) {
  const [name, setName] = useState("");
  const [content, setContent] = useState("");
  const [targetGroup, setTargetGroup] = useState<CustomBroadcast["targetGroup"]>("main");
  return <ConfirmModal title="Tambah BC custom" onClose={onClose}><div className="card-body"><label className="field">Nama BC custom<input value={name} autoFocus maxLength={100} placeholder="Contoh: Promo akhir pekan" onChange={(event) => setName(event.target.value)} /></label><label className="field">Isi pesan<textarea value={content} rows={7} maxLength={4096} placeholder="Tulis pesan Telegram di sini..." onChange={(event) => setContent(event.target.value)} /></label><label className="field">Kirim ke<select value={targetGroup} onChange={(event) => setTargetGroup(event.target.value as CustomBroadcast["targetGroup"])}>{targetOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label></div><div className="modal-actions"><button className="btn btn-ghost" onClick={onClose}>Batal</button><button className="btn btn-primary" disabled={!name.trim()} onClick={() => onCreate(name, content, targetGroup)}>Tambah</button></div></ConfirmModal>;
}
